document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildCrmShortLinkHtml(lead, proposalUrl) {
    ...
    const shortUrl = window.templateHost ? `${window.templateHost.replace(/\/$/, '')}/website-mockup/${shortAlias}` : (lead.tinyUrl || (shortAlias ? `${base}/go/${shortAlias}` : ''));
    const displayLabel = window.templateHost ? `/website-mockup/${shortAlias}` : `/go/${shortAlias}`;
...
window.generateLink = async function () {
  ...
  try {
    ...
    const activeLink = window.templateHost ? `${window.templateHost.replace(/\/$/, '')}/website-mockup/${result.alias}` : shortUrl;
